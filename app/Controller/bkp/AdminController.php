...
class AdminController extends AppController
{
	...
	public function challenge_uploads()
	{
		...
		if(isset($_FILES['upl']) && $_FILES['upl']['error'] == 0)
		{
			...
			else
			{
				...
				$newfilename    =   $randomno . $_FILES["upl"]["name"];
	...
	public function ajax_checkavail()
	{
		...
		{
			$this->loadModel("Category");
			echo $childnameavail = $this->Category->CheckChildCategoryAvailable($_POST['checkavail']);
			exit();
		}
		if($_POST['mode']=="Parent")
		{
			$this->loadModel("Category");
			echo $parentnameavail = $this->Category->CheckParentCategoryAvailable($_POST['checkavail']);
		}
